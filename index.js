...
const DocumentModel = require("hydrooj/src/model/document");
const documentCollection = hydrooj_1.db.collection('document');
function getRankingVisibilityFilter(viewer) {
    if (viewer.hasPerm(hydrooj_1.PERM.PERM_VIEW_PROBLEM_HIDDEN))
        return {};
    return { hidden: false };
}
function getProfileVisibilityFilter(viewer, targetUid) {
    if (viewer.hasPerm(hydrooj_1.PERM.PERM_VIEW_PROBLEM_HIDDEN) || viewer._id === targetUid)
        return {};
    return { hidden: false };
}
async function getCreatedProblems(domainId, uid, viewer) {
    return hydrooj_1.ProblemModel.getMulti(domainId, {
        owner: uid,
        ...getProfileVisibilityFilter(viewer, uid),
    }, hydrooj_1.ProblemModel.PROJECTION_LIST).toArray();
}
async function getAuthorRankRows(domainId, viewer, page) {
    const pageSize = hydrooj_1.SystemModel.get('pagination.ranking') || 50;
    const match = {
        domainId,
    ...
    const udict = rows.length ? await hydrooj_1.UserModel.getList(domainId, rows.map((row) => row._id)) : {};
...
async function getSelfAuthorStats(domainId, viewer) {
    if (!viewer.hasPriv(hydrooj_1.PRIV.PRIV_USER_PROFILE))
        return null;
    const match = {
        domainId,
        docType: DocumentModel.TYPE_PROBLEM,
...
class AuthorRankingHandler extends hydrooj_1.Handler {
    async get(domainId) {
...
async function extendUserDetail(handler) {
    if (handler.response.template !== 'user_detail.html')
        return;
...
async function apply(ctx) {
    ctx.Route('author_ranking', '/ranking/author', AuthorRankingHandler, hydrooj_1.PERM.PERM_VIEW_RANKING);
    ctx.injectUI('Nav', 'author_ranking', { prefix: 'author_ranking', before: 'ranking' }, hydrooj_1.PERM.PERM_VIEW_RANKING);
